...
auto clearGpuBarrierScope(Barrier barrier, int)
    -> decltype(barrier.setScopeAttr(nullptr), void()) {
  barrier.setScopeAttr(nullptr);
}
template <typename Barrier>
void clearGpuBarrierScope(Barrier, long) {}
...
struct BarrierOpConversion : OpRewritePattern<triton::gpu::BarrierOp> {
  ...
  LogicalResult matchAndRewrite(triton::gpu::BarrierOp op,
    ...
    // Newer LLVM gpu.barrier has inherent `scope` (#gpu.barrier_scope<workgroup>).
    // AscendNPU-IR's gpu dialect cannot parse that attribute. LLVM 3.7's
    // gpu.barrier has no scope (only optional address_spaces); skip then.
    clearGpuBarrierScope(barrier, 0);
    return success();
  }
};
