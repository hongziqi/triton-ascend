...
module {
  func.func @kernel(%arg0: memref<64xf32>) attributes {hacc.function_kind = #hacc.function_kind<DEVICE>} {
    ...
    %2 = scope.scope : () -> tensor<64xf32> {
      %4 = linalg.generic {indexing_maps = [#map], iterator_types = ["parallel"]} outs(%1 : tensor<64xf32>) {
      ^bb0(%out: f32):
        linalg.yield %cst : f32
      } -> tensor<64xf32>
    ...
    %3 = linalg.generic {indexing_maps = [#map, #map], iterator_types = ["parallel"]} ins(%2 : tensor<64xf32>) outs(%empty : tensor<64xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %cst : f32
    } -> tensor<64xf32>
