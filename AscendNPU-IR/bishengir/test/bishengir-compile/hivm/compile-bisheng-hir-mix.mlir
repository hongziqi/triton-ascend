...
// RUN: bishengir-compile -enable-lir-compile=false %s -o %t.ll
// RUN: bishengir-compile -enable-lir-compile=false %s -o %t.ll
...
module @M attributes {hivm.func_core_type = #hivm.func_core_type<MIX>} {
  // CHECK-AIC: ModuleID = 'LLVMDialectModule'
  // CHECK-AIC: @mul_add_mix_aic
  ...
  // CHECK-AIV: ModuleID = 'LLVMDialectModule'
  // CHECK-AIV: @mul_add_mix_aiv
