...
__aicore__ __attribute__((always_inline)) TYPE simt_cos(TYPE src) {
  float x = src;
  x = __fma(x, 0.0f, x);
  int quadrant = 0;
...
extern "C" {
REGISTER_SIMT_OP(cos, bfloat16_t, bfloat16_t);
REGISTER_SIMT_OP(cos, half, half);
REGISTER_SIMT_OP(cos, float, float);
}
#endif
