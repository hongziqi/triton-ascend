...
public:
                        ...
                        uint32_t sUbOffset, uint32_t nIdx, uint32_t &glFlag)
    {
        ...
        // input QK
        AscendCBisheng::DataCopy(lsUbTensor[sUbOffset], gInput, AscendCBisheng::DataCopyParams(m, nStride / FLOAT_BLOCK_SIZE, 0, 0));
        ...
        if (nIdx == 0) {
            AscendCBisheng::DataCopy(hmUbTensor[rowOffset], lmUbTensor[rowOffset],
                              AscendCBisheng::DataCopyParams(1, round_m / FLOAT_BLOCK_SIZE, 0, 0));
            AscendC::PipeBarrier<PIPE_V>();
        } else {
            SetVecMask(m);
            // *** hm = vmax(lm, gm)
        ...
        AscendCBisheng::DataCopy(gOutput, lpUbTensor[sUbOffset * 2], AscendCBisheng::DataCopyParams(m, nStride * 2 / 32, 0, 0));
        AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::MTE3_MTE2>(pingpongFlag);
        if (nIdx == 0) {
            // *** gl = ll
            AscendCBisheng::DataCopy(glUbTensor[rowOffset], llUbTensor[rowOffset],
                              AscendCBisheng::DataCopyParams(1, round_m / FLOAT_BLOCK_SIZE, 0, 0));
            AscendC::PipeBarrier<PIPE_V>();
        } else {
            SetVecMask(m);
