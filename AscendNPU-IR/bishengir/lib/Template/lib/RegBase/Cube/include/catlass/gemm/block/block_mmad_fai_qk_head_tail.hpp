...
public:
        ...
        uint32_t &nkblockLoop, bool firstItr, bool endItr, bool initMmad, bool firstQItr, bool endQItr, bool preloadFlag)
    {
        ...
        if constexpr (!ENABLE_UNIT_FLAG_) {
            if (firstItr) {
                AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::FIX_M>(locPingPongFlag);
            }
        } else {
            unitFlag = 0b11;
        }
        ...
        if (endItr) {
            if constexpr (!ENABLE_UNIT_FLAG_) {
                AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::M_FIX>(locPingPongFlag);
                AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::M_FIX>(locPingPongFlag);
                copyL0CToGm(gC, l0CTensor[locPingPongFlag * mRound * 128], layoutC, layoutInL0C);
                AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::FIX_M>(locPingPongFlag);
            } else {
