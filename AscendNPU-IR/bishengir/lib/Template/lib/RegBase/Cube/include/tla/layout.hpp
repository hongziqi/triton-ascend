...
struct Layout : private tla::tuple<Shape, Stride> {
  ...
  CATLASS_HOST_DEVICE constexpr Layout(Shape const &shape = {},
                                       Stride const &stride = {})
  ...
  template <int... I> CATLASS_HOST_DEVICE constexpr decltype(auto) shape() {
  ...
  template <int... I> CATLASS_HOST_DEVICE constexpr decltype(auto) stride() {
...
CATLASS_HOST_DEVICE constexpr auto MakeMxScaleLayout(T const &rows,
                                                     U const &cols) {
  static_assert(
      std::is_same_v<Element, AscendCBisheng::fp8_e8m0_t> &&
  ...
  if constexpr (std::is_same_v<LayoutTag, Catlass::layout::RowMajor>) {
    if constexpr (!isMxScaleB) {
      return MakeLayout(
          MakeShape(rows, MakeShape(Int<ELE_NUM_PER_C0>{},
                                    CeilDiv(cols, Int<ELE_NUM_PER_C0>{}))),
          MakeStride(RoundUp(cols, Int<ELE_NUM_PER_C0>{}),
                     MakeStride(Int<1>{}, Int<ELE_NUM_PER_C0>{})));
    } else {
      return MakeLayout(
          MakeShape(MakeShape(Int<ELE_NUM_PER_C0>{},
                              CeilDiv(rows, Int<ELE_NUM_PER_C0>{})),
                    cols),
          MakeStride(MakeStride(Int<1>{}, cols * ELE_NUM_PER_C0),
                     Int<ELE_NUM_PER_C0>{}));
    }
  } else if constexpr (std::is_same_v<LayoutTag,
    ...
    if constexpr (!isMxScaleB) {
      return MakeLayout(
          ...
          MakeStride(Int<ELE_NUM_PER_C0>{},
                     MakeStride(Int<1>{}, rows * ELE_NUM_PER_C0)));
    } else {
      return MakeLayout(
          MakeShape(MakeShape(Int<ELE_NUM_PER_C0>{},
                              CeilDiv(rows, Int<ELE_NUM_PER_C0>{})),
                    cols),
          MakeStride(MakeStride(Int<1>{}, Int<ELE_NUM_PER_C0>{}),
                     RoundUp(rows, Int<ELE_NUM_PER_C0>{})));
    }
  } else if constexpr (std::is_same_v<LayoutTag, Catlass::layout::zZ>) {
    return MakeLayout(
        MakeShape(MakeShape(Int<Catlass::C0_NUM_PER_FRACTAL>{},
                            CeilDiv(rows, Int<Catlass::C0_NUM_PER_FRACTAL>{})),
                  MakeShape(Int<ELE_NUM_PER_C0>{},
                            CeilDiv(cols, Int<ELE_NUM_PER_C0>{}))),
        MakeStride(MakeStride(Int<ELE_NUM_PER_C0>{},
                              RoundUp((int64_t)cols, Int<ELE_NUM_PER_C0>{}) *
    ...
    return MakeLayout(
        MakeShape(MakeShape(Int<ELE_NUM_PER_C0>{},
                            CeilDiv(rows, Int<ELE_NUM_PER_C0>{})),
                  MakeShape(Int<Catlass::C0_NUM_PER_FRACTAL>{},
                            CeilDiv(cols, Int<Catlass::C0_NUM_PER_FRACTAL>{}))),
        MakeStride(MakeStride(Int<1>{}, Int<ELE_NUM_PER_FRACTAL>{}),
                   MakeStride(Int<ELE_NUM_PER_C0>{},
                              RoundUp((int64_t)rows, Int<ELE_NUM_PER_C0>{}) *
                                  Catlass::C0_NUM_PER_FRACTAL)));
  }
}
...
template <class Shape, class Stride>
CATLASS_HOST_DEVICE constexpr auto MakeLayout(Shape const &shape,
                                              Stride const &stride) {
...
CATLASS_HOST_DEVICE constexpr auto MakeLayoutFromTag(LayoutTag const &tag) {
  static_assert(std::is_same_v<LayoutTag, Catlass::layout::RowMajor> ||
                ...
                "Unsupported LayoutTag for MakeLayoutFromTag, only support "
                "Catlass::layout::RowMajor or"
                "Catlass::layout::ColumnMajor or Catlass::layout::zN or "
                "Catlass::layout::nZ");
  ...
  if constexpr (std::is_same_v<LayoutTag, Catlass::layout::RowMajor>) {
    ...
    return MakeLayout(MakeShape(MakeShape(tag.shape(0), tag.shape(1)),
                                MakeShape(tag.shape(2), tag.shape(3))),
                      MakeStride(MakeStride(tag.stride(0), tag.stride(1)),
                                 MakeStride(tag.stride(2), tag.stride(3))));
  }
}

// Return the shape of a mode
template <int... Is, class Shape, class Stride>
CATLASS_HOST_DEVICE constexpr decltype(auto)
...
CATLASS_HOST_DEVICE constexpr auto
crd2offset(Coord const &coord, Shape const &shape, Stride const &stride);

namespace detail {

template <class Coord, class Shape, class Stride, int... Is>
CATLASS_HOST_DEVICE constexpr auto
crd2offset_ttt(Coord const &coord, Shape const &shape, Stride const &stride,
               seq<Is...>) {
...
CATLASS_HOST_DEVICE constexpr auto
crd2offset_itt(CInt const &coord, STuple const &shape, DTuple const &stride,
               seq<I0, Is...>) {
  if constexpr (sizeof...(Is) ==
    ...
    return crd2offset(coord % Product{}(get<I0>(shape)), get<I0>(shape),
                      get<I0>(stride)) +
           crd2offset_itt(coord / Product{}(get<I0>(shape)), shape, stride,
                          seq<Is...>{});
  }
}

template <class Element, class Layout, class Enable = void> struct isMxScalezZ {
  static bool const value = false;
};

template <class Layout>
struct isMxScalezZ<
    AscendCBisheng::fp8_e8m0_t, Layout,
    std::enable_if_t<Layout::depth == 2 && Layout::rank == 2 &&
                     rank_v<decltype(shape<0>(Layout{}))> == 2 &&
  ...
  static bool const value =
      (shape<0, 0>(Layout{}) == Catlass::C0_NUM_PER_FRACTAL &&
       shape<1, 0>(Layout{}) == ELE_NUM_PER_C0 && stride<1, 0>(Layout{}) == 1 &&
       stride<1, 1>(Layout{}) == ELE_NUM_PER_FRACTAL);
};

template <class Element, class Layout, class Enable = void> struct isMxScalenN {
  static bool const value = false;
};
...
struct isMxScalenN<
  ...
  static constexpr uint32_t ELE_NUM_PER_FRACTAL = 32;
  static bool const value =
      (shape<0, 0>(Layout{}) == ELE_NUM_PER_C0 &&
...
template <class Layout>
struct isVector<Layout,
                std::enable_if_t<Layout::depth == 1 && Layout::rank == 1>> {
...
template <class Layout>
struct isRowMajor<Layout,
                  std::enable_if_t<Layout::depth == 1 && Layout::rank == 2>> {
...
template <class Layout>
struct isColumnMajor<
    Layout, std::enable_if_t<Layout::depth == 1 && Layout::rank == 2>> {
  static bool const value = (stride<0>(Layout{}) == 1);
};
...
struct iszN<Element, Layout,
  ...
  static constexpr uint32_t ELE_NUM_PER_C0 =
      Catlass::BYTE_PER_C0 / sizeof(Element);
  static constexpr uint32_t ELE_NUM_PER_FRACTAL =
      Catlass::BYTE_PER_FRACTAL / sizeof(Element);
  static bool const value =
      (shape<0, 0>(Layout{}) == Catlass::C0_NUM_PER_FRACTAL &&
       shape<1, 0>(Layout{}) == ELE_NUM_PER_C0 && stride<1, 0>(Layout{}) == 1 &&
       stride<0, 1>(Layout{}) == ELE_NUM_PER_FRACTAL);
...
template <class T> CATLASS_HOST_DEVICE constexpr auto MakeLayout(T const &len) {
...
CATLASS_HOST_DEVICE constexpr auto MakeLayout(T const &rows, U const &cols) {
  static_assert(std::is_same_v<LayoutTag, Catlass::layout::RowMajor> ||
                ...
                "Catlass::layout::RowMajor or"
                "Catlass::layout::ColumnMajor or Catlass::layout::zN or "
                "Catlass::layout::nZ or Catlass::layout::zZ");
  ...
  if constexpr (std::is_same_v<LayoutTag, Catlass::layout::RowMajor>) {
    ...
    return MakeLayout(
        ...
        MakeStride(
            ...
            MakeStride(Int<1>{}, RoundUp((int64_t)rows,
                                         Int<Catlass::C0_NUM_PER_FRACTAL>{}) *
                                     ELE_NUM_PER_C0)));
  } else if constexpr (std::is_same_v<LayoutTag, Catlass::layout::zZ>) {
    return MakeLayout(
        MakeShape(MakeShape(Int<Catlass::C0_NUM_PER_FRACTAL>{},
    ...
    return MakeLayout(
        MakeShape(MakeShape(Int<ELE_NUM_PER_C0>{},
                  ...
                  MakeShape(Int<Catlass::C0_NUM_PER_FRACTAL>{},
                            CeilDiv(cols, Int<Catlass::C0_NUM_PER_FRACTAL>{}))),
        MakeStride(
            MakeStride(Int<1>{}, RoundUp((int64_t)cols,
                                         Int<Catlass::C0_NUM_PER_FRACTAL>{}) *
                                     ELE_NUM_PER_C0),
            MakeStride(Int<ELE_NUM_PER_C0>{}, Int<ELE_NUM_PER_FRACTAL>{})));
  }
...
CATLASS_HOST_DEVICE constexpr auto MakeLayoutTile(Layout const &layout,
                                                  ShapeNew const &shapeNew) {
  static_assert(is_tuple<ShapeNew>::value && depth_v<ShapeNew> == 1 &&
                (rank_v<ShapeNew> == 1 || rank_v<ShapeNew> == 2));

  if constexpr (Layout::depth == 1 &&
                (Layout::rank == 1 || Layout::rank == 2)) {
    ...
    return MakeLayout(MakeShape(MakeShape(Int<dstInnerShapeRow>{},
                                          CeilDiv<dstInnerShapeRow>(rows)),
                                MakeShape(Int<dstInnerShapeCol>{},
                                          CeilDiv<dstInnerShapeCol>(cols))),
                      layout.stride());
  } else {
    const uint32_t rows = get<0>(shapeNew);
    const uint32_t cols = get<1>(shapeNew);
