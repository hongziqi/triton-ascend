 ...
 public:
         ...
         uint32_t isFirstStackTile)
     {
         if (isFirstStackTile) {
             AscendCBisheng::DataCopy(
                 ...
                 AscendCBisheng::DataCopyParams(1, rowNumCurLoopRound / FLOAT_BLOCK_SIZE, 0, 0));
             AscendC::PipeBarrier<PIPE_V>();
         } else {
         ...
         AscendC::PipeBarrier<PIPE_V>();
         // *** gm = hm
         AscendCBisheng::DataCopy(gmUbTensor[rowOffset], hmUbTensor[rowOffset],
                           AscendCBisheng::DataCopyParams(1, rowNumCurLoopRound / FLOAT_BLOCK_SIZE, 0, 0));
         AscendC::PipeBarrier<PIPE_V>();
     }
                         ...
                         uint32_t curStackTileMod)
     {
         ...
         if constexpr (maskCat == MaskCategory::NO_MASK){
             AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::MTE3_V>(pingpongFlag);
         }

         DownCastP(sUbOffset, rowNumCurLoop, columnNumRound);
         AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::V_MTE3>(pingpongFlag);

         CalcLocalRowSum(sUbOffset, rowNumCurLoopRound, columnNum, columnNumRound, rowOffset);
         AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::V_MTE2>(pingpongFlag);

         AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::V_MTE3>(pingpongFlag);
         CopyPUbToGm(gOutput, sUbOffset, rowNumCurLoop, columnNumRound, columnNumPad);
         if constexpr (maskCat == MaskCategory::NO_MASK){
             AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::MTE3_V>(pingpongFlag);
         }
         else if constexpr (maskCat == MaskCategory::CAUSAL_MASK){
             AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::MTE3_V>(EVENT_ID0);
             AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::MTE3_V>(EVENT_ID1);
         }
                     ...
                     uint32_t isFirstStackTile, uint32_t qSBlockSize, uint32_t qNBlockSize, uint32_t curStackTileMod)
     {
         ...
         for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoopNum + preLoad; rowLoopIdx++) {
             ...
             // loop 0 mask load before cross core sync
                ...
                AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::V_MTE2>(pingpongFlag);
                CopySGmToUb(gInputCurLoop, (pingpongFlag * MAX_UB_S_ELEM_NUM), rowNumCurLoop, columnNumRound, columnNumPad);
                AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::MTE2_V>(pingpongFlag);
            ...
            if(rowLoopIdx >= preLoad){
                ...
                AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::MTE2_V>(pingpongFlag);
                    ...
                    Arch::CrossCoreFlag qkReady)
     {
        ...
        for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoopNum + preLoad; rowLoopIdx++) {
            ...
            if(rowLoopIdx >= preLoad){
                ...
                AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::MTE3_V>(EVENT_ID1);
