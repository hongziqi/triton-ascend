...
#define DECLARE_SET2D_FP8(dst_scope, dst_dim, type)                            \
  __aicore__                                                                   \
      __attribute__((always_inline)) void _mlir_ciface_set_l1_2d_##type(       \
          int8_t scalar, memref_t<__##dst_scope##__ type, dst_dim> *dst)
#define REGISTE_SET2D_FP8(dst_scope, dst_dim, type)                            \
  DECLARE_SET2D_FP8(dst_scope, dst_dim, type) {                                \
    set_##dst_scope##_to_2d_core_fp8<type>(scalar, dst);                       \
  }
#endif
