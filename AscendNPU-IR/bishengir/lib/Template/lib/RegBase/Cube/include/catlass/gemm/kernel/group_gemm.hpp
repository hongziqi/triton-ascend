    ...
    void operator()<AscendC::AIC>(Params &params)
    {
        ...
        for (uint32_t groupIdx = 0; groupIdx < params.problemCount; ++groupIdx) {
            ...
            for (uint32_t i = 0; i < l0CBlockNum; i++) {
                AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::FIX_M>((int32_t)i);
            }
            ...
            for (uint32_t loopIdx = startLoopIdx; loopIdx < coreLoops; loopIdx += AscendC::GetBlockNum()) {
                ...
                AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::FIX_M>((int32_t)singleIdx);
                ...
                AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::FIX_M>((int32_t)singleIdx);
            ...
            #pragma unroll
            for (uint32_t i = 0; i < l0CBlockNum; i++) {
                AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::FIX_M>((int32_t)i);
            }
        }
