        ...
        uint32_t nIdx, uint32_t isLast)
    {
        ...
        if (subM > 0) {
            ...
            if (nIdx != 0) {
                ...
                // gl = ll
                AscendCBisheng::DataCopy(glUbTensor, llUbTensor[nIdx % MULTIPLIER * FLOAT_ELENUM_PER_LINE],
                    AscendCBisheng::DataCopyParams(1, subMRound / FLOAT_ELENUM_PER_BLK, 0, 0));
                AscendC::PipeBarrier<PIPE_V>();
                if (goFlag == 1) {
                    AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::MTE3_V>(EVENT_ID0);
                    goFlag = 0;
                }
                AscendCBisheng::DataCopy(goUbTensor, loUbTensor,
                    AscendCBisheng::DataCopyParams(1, subM * kRound / FLOAT_ELENUM_PER_BLK, 0, 0));
                AscendC::PipeBarrier<PIPE_V>();
            }
            AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::V_MTE2>(EVENT_ID1);
            if (isLast) {
                ...
                // copy O to GM
                AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::V_MTE3>(EVENT_ID1);
                AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::V_MTE3>(EVENT_ID1);
