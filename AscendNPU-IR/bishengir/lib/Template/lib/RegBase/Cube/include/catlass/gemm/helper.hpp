...
template <class Element, class Layout> struct L1AlignHelper {
  static_assert(DEPENDENT_FALSE<Element>,
...
template <class Element> struct L1AlignHelper<Element, layout::NC1HWC0> {
  static constexpr uint32_t ELE_NUM_PER_C0 = BYTE_PER_C0 / sizeof(Element);
  static constexpr uint32_t HOWO_ALIGNED = C0_NUM_PER_FRACTAL;
};

template <class Element> struct L1AlignHelper<Element, layout::CI1KHKWCOCI0> {
  static constexpr uint32_t ELE_NUM_PER_C0 = BYTE_PER_C0 / sizeof(Element);
  static constexpr uint32_t COUT_ALIGNED = C0_NUM_PER_FRACTAL;
};

template <class Element_, bool isMx = false> struct GetL0Element {
  using Element = Element_;
};
#if defined(__NPU_ARCH__) && __NPU_ARCH__ == 3101
template <> struct GetL0Element<float8_e4m3_t, true> {
  using Element = AscendCBisheng::mx_fp8_e4m3_t;
};

template <> struct GetL0Element<float8_e5m2_t, true> {
  using Element = AscendCBisheng::mx_fp8_e5m2_t;
};
#endif
...
template <> struct ElementAccumulatorSelector<float8_e4m3_t, float8_e4m3_t> {
  using ElementAccumulator = float;
};
...
struct L1ATypeSelector<Gemm::GemmType<Element, layout::RowMajor>> {
  using L1AType =
      Gemm::GemmType<Element, layout::zN, AscendCBisheng::TPosition::A1>;
};
...
struct L1ATypeSelector<Gemm::GemmType<Element, layout::ColumnMajor>> {
  using L1AType =
      Gemm::GemmType<Element, layout::nZ, AscendCBisheng::TPosition::A1>;
};
...
struct L1ATypeSelector<Gemm::GemmType<Element, layout::NDC1HWC0>> {
  using L1AType =
      Gemm::GemmType<Element, layout::NDC1HWC0, AscendCBisheng::TPosition::A1>;
};

template <class Element>
struct L1ATypeSelector<Gemm::GemmType<Element, layout::NC1HWC0>> {
  using L1AType =
      Gemm::GemmType<Element, layout::NC1HWC0, AscendCBisheng::TPosition::A1>;
};

template <class GmBType> struct L1BTypeSelector {
  static_assert(
      DEPENDENT_FALSE<GmBType>,
      "Unsupported layout selector, can not find the specialization.");
};
...
struct L1BTypeSelector<Gemm::GemmType<Element, layout::CI1KHKWCOCI0>> {
  using L1BType = Gemm::GemmType<Element, layout::CI1KHKWCOCI0,
...
struct L1BiasTypeSelector<Gemm::GemmType<Element, layout::VectorLayout>, ElementAccumulator> {
    using GMBiasType = Gemm::GemmType<Element, layout::VectorLayout, AscendCBisheng::TPosition::GM>;
    using L1BiasType = Gemm::GemmType<Element, layout::VectorLayout, AscendCBisheng::TPosition::A1>;
    using L0BiasType = Gemm::GemmType<ElementAccumulator, layout::VectorLayout, AscendCBisheng::TPosition::C2>;
};

template <class ArchTag> struct L0ALayoutSelector {
  static_assert(
      DEPENDENT_FALSE<ArchTag>,
      "Unsupported layout selector, can not find the specialization.");
};

template <> struct L0ALayoutSelector<Arch::AtlasA2> {
  using Layout = layout::zZ;
};

template <> struct L0ALayoutSelector<Arch::AtlasA5> {
...
struct L1AlignHelperTla {
  static_assert(
      DEPENDENT_FALSE<Element>,
      "Unsupported align helper tla, can not find the specialization.");
};

template <class Element, class Layout>
struct L1AlignHelperTla<
    Element, Layout, std::enable_if_t<tla::detail::isRowMajor<Layout>::value>> {
  static constexpr uint32_t ELE_NUM_PER_C0 = BYTE_PER_C0 / sizeof(Element);
  static constexpr uint32_t M_ALIGNED = C0_NUM_PER_FRACTAL;
  static constexpr uint32_t K_ALIGNED = ELE_NUM_PER_C0;
  static constexpr uint32_t N_ALIGNED = ELE_NUM_PER_C0;
...
struct L1AlignHelperTla<
    Element, Layout,
    std::enable_if_t<tla::detail::isColumnMajor<Layout>::value>> {
  static constexpr uint32_t ELE_NUM_PER_C0 = BYTE_PER_C0 / sizeof(Element);
  static constexpr uint32_t M_ALIGNED = ELE_NUM_PER_C0;
  static constexpr uint32_t K_ALIGNED = ELE_NUM_PER_C0;
  static constexpr uint32_t N_ALIGNED = C0_NUM_PER_FRACTAL;
...
struct L1AndL0TypeSelectorGemm<Gemm::GemmType<Element, layout::RowMajor>, Gemm::GemmType<Element, layout::RowMajor>>{
    using L1AType = Gemm::GemmType<Element, layout::zN, AscendCBisheng::TPosition::A1>;
    using L1BType = Gemm::GemmType<Element, layout::zZ, AscendCBisheng::TPosition::B1>;
    using L0AType = Gemm::GemmType<Element, layout::zZ, AscendCBisheng::TPosition::A2>;
    using L0BType = Gemm::GemmType<Element, layout::nZ, AscendCBisheng::TPosition::B2>;
};

template<>
struct L1AndL0TypeSelectorGemm<Gemm::GemmType<int8_t, layout::RowMajor>, Gemm::GemmType<int8_t, layout::RowMajor>>{
    using L1AType = Gemm::GemmType<int8_t, layout::zN, AscendCBisheng::TPosition::A1>;
    using L1BType = Gemm::GemmType<int8_t, layout::zN, AscendCBisheng::TPosition::B1>;
    using L0AType = Gemm::GemmType<int8_t, layout::zZ, AscendCBisheng::TPosition::A2>;
    using L0BType = Gemm::GemmType<int8_t, layout::nZ, AscendCBisheng::TPosition::B2>;
};

template<class Element>
struct L1AndL0TypeSelectorGemm<Gemm::GemmType<Element, layout::ColumnMajor>, Gemm::GemmType<Element, layout::ColumnMajor>>{
    using L1AType = Gemm::GemmType<Element, layout::nN, AscendCBisheng::TPosition::A1>;
    using L1BType = Gemm::GemmType<Element, layout::nZ, AscendCBisheng::TPosition::B1>;
    using L0AType = Gemm::GemmType<Element, layout::zZ, AscendCBisheng::TPosition::A2>;
    using L0BType = Gemm::GemmType<Element, layout::nZ, AscendCBisheng::TPosition::B2>;
};
...
struct L1AndL0TypeSelectorGemm<Gemm::GemmType<int8_t, layout::ColumnMajor>, Gemm::GemmType<int8_t, layout::ColumnMajor>>{
    using L1AType = Gemm::GemmType<int8_t, layout::nZ, AscendCBisheng::TPosition::A1>;
    using L1BType = Gemm::GemmType<int8_t, layout::nZ, AscendCBisheng::TPosition::B1>;
    using L0AType = Gemm::GemmType<int8_t, layout::zZ, AscendCBisheng::TPosition::A2>;
    using L0BType = Gemm::GemmType<int8_t, layout::nZ, AscendCBisheng::TPosition::B2>;
};
