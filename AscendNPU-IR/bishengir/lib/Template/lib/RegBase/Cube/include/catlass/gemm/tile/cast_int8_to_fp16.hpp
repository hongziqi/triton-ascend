...
struct TileCastInt8ToFp16Dequant {
    ...
    TileCastInt8ToFp16Dequant(Arch::Resource<ArchTag> const &resource, Params const &params_) : params(params_)
    {
        if constexpr (g_coreType == AscendC::AIV) {
            ...
            for (uint32_t i = 0; i < STAGES; i++) {
                ...
                AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::MTE3_V>(ubEventList[i]);
            }
        }
    }
    ...
    ~TileCastInt8ToFp16Dequant() {
        if constexpr (g_coreType == AscendC::AIV) {
            for (uint32_t i = 0; i < STAGES; i++) {
                ...
                AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::MTE3_V>(ubEventList[i]);
            }
        }
    }
    ...
    )
    {
        ...
        for (uint32_t loopIdx = 0; loopIdx < loops; loopIdx++) {
            ...
            AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::MTE2_V>(ubEventList[pingpong]);
            AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::MTE2_V>(ubEventList[pingpong]);

            AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::MTE3_V>(ubEventList[pingpong]);
            ...
            AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::MTE3_V>(ubEventList[pingpong]);
