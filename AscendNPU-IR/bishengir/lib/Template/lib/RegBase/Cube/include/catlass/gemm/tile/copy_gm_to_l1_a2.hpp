        ...
        LayoutDst const &layoutDst, LayoutSrc const &layoutSrc)
    {
        ...
        if (layoutSrc.shape(0) <= 16) {
            ...
            for (int i = 0; i < layoutSrc.shape(0); ++i) {
                AscendCBisheng::DataCopyParams dataCopyParams(
            ...
            if (layoutSrc.stride(0) < STRIDE_LIMIT) {
                if (layoutSrc.shape(1) != ELE_NUM_PER_C0 || layoutSrc.stride(0) != ELE_NUM_PER_C0) {
                    ...
                    // transfers.
                    AscendCBisheng::DataCopy(dstTensor, srcTensor, layoutSrc.shape(0) * layoutSrc.shape(1));
                }
            } else {
                ...
                for (uint32_t i = 0; i < layoutSrc.shape(0); i++) {
                    AscendCBisheng::DataCopy(dstTensor[i * ELE_NUM_PER_C0], srcTensor[i * layoutSrc.stride(0)], intriParams);
                }
            }
        }
    }
        ...
        if (layoutSrc.shape(1) <= 16) {
            ...
            if (layoutSrc.stride(1) < STRIDE_LIMIT) {
                ...
                for (uint32_t i = 0; i < layoutSrc.shape(1); i++) {
                    AscendCBisheng::DataCopy(dstTensor[i * ELE_NUM_PER_C0], srcTensor[i * layoutSrc.stride(1)], intriParams);
                }
            }
        }
    }
        ...
        LayoutDst const &layoutDst, LayoutSrc const &layoutSrc)
    {
        ...
        if (layoutSrc.shape(0) == 1) {
            ...
            AscendCBisheng::DataCopyParams dataCopyParams(
                ...
                0,
                (layoutDst.stride(3) - layoutDst.shape(2)) / ELE_NUM_PER_C0);
            AscendCBisheng::DataCopy(dstTensor, srcTensor, dataCopyParams);
        } else {
            if (layoutSrc.stride(0) < STRIDE_LIMIT) {
                if (layoutSrc.shape(1) != ELE_NUM_PER_C0 || layoutSrc.stride(0) != ELE_NUM_PER_C0) {
                    intriParams.nValue = layoutSrc.shape(0);
        ...
        uint32_t dstNzC0Stride)
    {
        ...
        if (srcNdMatrixStride < STRIDE_LIMIT) {
            ...
            for (uint32_t i = 0; i < ndNum; i++) {
                AscendCBisheng::DataCopy(dstTensor[i * ELE_NUM_PER_C0], srcTensor[i * srcNdMatrixStride], intriParams);
            }
        }
    }
};
...
struct CopyGmToL1IntervalDataCopy<Arch::AtlasA2, Gemm::GemmType<half, layout::RowMajor>> {
        ...
        LayoutDst const &layoutDst, LayoutSrc const &layoutSrc)
    {
        for (int i = 0; i < layoutSrc.shape(0); ++i) {
            ...
            AscendCBisheng::DataCopy(dstTensor[i * layoutDst.shape(2)], srcTensor[i * layoutSrc.stride(0)], dataCopyParams);
        }
    }
};
...
struct CopyGmToL1IntervalDataCopy<Arch::AtlasA2, Gemm::GemmType<half, layout::ColumnMajor>> {
        ...
        LayoutDst const &layoutDst, LayoutSrc const &layoutSrc)
    {
        for (int i = 0; i < layoutSrc.shape(1); ++i) {
            ...
            AscendCBisheng::DataCopy(dstTensor[i * layoutDst.shape(0)], srcTensor[i * layoutSrc.stride(1)], dataCopyParams);
        }
    }
};
...
struct CopyGmToL1IntervalDataCopy<Arch::AtlasA2, Gemm::GemmType<half, layout::PaddingColumnMajor>> {
        ...
        LayoutDst const &layoutDst, LayoutSrc const &layoutSrc)
    {
        for (int i = 0; i < layoutSrc.orgShape(1); ++i) {
            ...
            AscendCBisheng::DataCopy(dstTensor[i * layoutDst.shape(0)], srcTensor[i * layoutSrc.stride(2)], dataCopyParams);
        }
    }
};
...
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::RowMajor>, Gemm::GemmType<Element, layout::zN, AscendCBisheng::TPosition::A1>> {
    using LayoutDst = layout::zN;
    using LayoutSrc = layout::RowMajor;
...
template <class ArchTag, class Element>
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::RowMajor>, Gemm::GemmType<Element, layout::zZ, AscendCBisheng::TPosition::B1>> {
        ...
        LayoutDst const &layoutDst, LayoutSrc const &layoutSrc)
    {
        ...
        if (srcNdStride < STRIDE_LIMIT) {
            ...
            if (remains) {
                ...
                AscendCBisheng::DataCopy(dstTensor[ndNum * layoutDst.stride(1)], srcTensor[ndNum * srcNdStride], tailParams);
            }
            ...
            for (uint32_t i = 0; i < ndNum; i++) {
                ...
                AscendCBisheng::DataCopy(dstTensor[i * layoutDst.stride(1)], srcTensor[i * srcNdStride], intriParams);
            }
            if (remains) {
                AscendC::Nd2NzParams tailParams;
                tailParams.ndNum = 1;
            ...
            for (uint32_t i = 0; i < layoutSrc.shape(0); i++) {
                ...
                AscendCBisheng::DataCopy(dstTensor[offsetDst], srcTensor[offsetSrc], intriParams);
            }
        }
    }
};
...
template <class ArchTag, class Element>
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::ColumnMajor>, Gemm::GemmType<Element, layout::nN, AscendCBisheng::TPosition::A1>> {
        ...
        LayoutDst const &layoutDst, LayoutSrc const &layoutSrc)
    {
        ...
        if (srcNdStride < STRIDE_LIMIT) {
            ...
            if (remains) {
                ...
                AscendCBisheng::DataCopy(dstTensor[ndNum * layoutDst.stride(3)], srcTensor[ndNum * srcNdStride], tailParams);
            }
            ...
            for (uint32_t i = 0; i < ndNum; i++) {
                ...
                AscendCBisheng::DataCopy(dstTensor[i * layoutDst.stride(3)], srcTensor[i * srcNdStride], intriParams);
            }
            if (remains) {
                AscendC::Nd2NzParams tailParams;
                tailParams.ndNum = 1;
...
template <class ArchTag, class Element>
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::ColumnMajor>, Gemm::GemmType<Element, layout::nZ, AscendCBisheng::TPosition::B1>> {
    using LayoutDst = layout::nZ;
    using LayoutSrc = layout::ColumnMajor;
...
template <class ArchTag, class Element>
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::ColumnMajor>, Gemm::GemmType<Element, layout::nZ, AscendCBisheng::TPosition::A1>> {
    using LayoutDst = layout::nZ;
    using LayoutSrc = layout::ColumnMajor;
...
template <class ArchTag, class Element>
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::VectorLayout>, Gemm::GemmType<Element, layout::zN, AscendCBisheng::TPosition::A1>> {
    using LayoutDst = layout::zN;
...
template <class ArchTag, class Element>
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::NDC1HWC0, AscendCBisheng::TPosition::GM>> {
            ...
            for (uint64_t i = 0; i < dataCopyLoop; i++) {
                if (srcStrideBeyondMaxU16) {
                    ...
                    for (uint64_t j = 0; j < dataCopySubLoop; j++) {
                        AscendCBisheng::DataCopy<Element>(dstTensor[aL1SubOffset], srcTensor[aL1GmSubOffset], repeatParams);
                        aL1GmSubOffset += OriH * OriW * OriK0;
                        aL1SubOffset += hiLoadL1 * OriW * OriK0;
                    }
                } else {
                    AscendCBisheng::DataCopy<Element>(dstTensor[aL1Offset], srcTensor[aL1GmOffset], repeatParams);
                }
...
template <class ArchTag, class Element>
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::KDC1KHKWN1N0C0, AscendCBisheng::TPosition::GM>> {
    using LayoutDst = layout::nZ;
        ...
        LayoutDst const &layoutDst, LayoutSrc const &layoutSrc)
    {
        ...
        if (currentNBL1 >= OriCoAlign) {
            ...
            if(repeatTimes > LOAD2D_MAX_REPEAT_TIMES) {
                ...
                AscendCBisheng::DataCopy<Element>(dstTensor, srcTensor, repeatParams);
            } else {
                AscendC::LoadData2DParams loadData2dParams;
                loadData2dParams.srcStride = 1;
                loadData2dParams.repeatTimes = repeatTimes;
                AscendCBisheng::LoadData<Element>(dstTensor, srcTensor, loadData2dParams);
            }
        } else {
...
template <class ArchTag, class Element>
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::ColumnMajor>, Gemm::GemmType<Element, layout::nN, AscendCBisheng::TPosition::B1>> {
    using LayoutDst = layout::nN;
    using LayoutSrc = layout::ColumnMajor;
...
template <class ArchTag, class Element>
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::RowMajor>, Gemm::GemmType<Element, layout::zN, AscendCBisheng::TPosition::B1>> {
    using LayoutDst = layout::zN;
    using LayoutSrc = layout::RowMajor;
...
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::zN>> {
        ...
        LayoutDst const &layoutDst, LayoutSrc const &layoutSrc)
    {
        ...
        if (layoutSrc.stride(3) / ELE_NUM_PER_C0 < STRIDE_LIMIT) {
            ...
            AscendCBisheng::DataCopy(dstTensor, srcTensor, repeatParams);
        } else {
            repeatParams.blockCount = 1;
            repeatParams.blockLen = blockLen;
            repeatParams.srcStride = 0;
            ...
            for (uint32_t i = 0; i < blockCount; i++) {
                ...
                AscendCBisheng::DataCopy(dstTensor[dstOffset], srcTensor[srcOffset], repeatParams);
            }
        }
    }
};
...
template <class Element>
struct CopyGmToL1<Arch::AtlasA2, Gemm::GemmType<Element, layout::RowMajor>,
    Gemm::GemmType<Element, layout::RowMajor, AscendCBisheng::TPosition::A1>> {
...
struct TileCopyTla<Arch::AtlasA2,
    ...
    void operator()(TensorDst const &dstTensor, TensorSrc const &srcTensor)
    {
        static_assert(tla::detail::isRowMajor<typename TensorSrc::Layout>::value &&
                      ...
                      TensorSrc::position == AscendCBisheng::TPosition::GM &&
                      TensorDst::position == AscendCBisheng::TPosition::A1,
        ...
        if (srcDValue < STRIDE_LIMIT) {
            ...
            for (uint32_t i = 0; i < nValue; i++) {
                AscendCBisheng::DataCopy(dstTensor.data()[dstOffset + i * ELE_NUM_PER_C0],
...
struct TileCopyTla<Arch::AtlasA2,
    ...
    void operator()(TensorDst const &dstTensor, TensorSrc const &srcTensor)
    {
        ...
        if (srcOuterStrideCol / ELE_NUM_PER_C0 <= STRIDE_LIMIT) {
            ...
            repeatParams.dstStride = 0;
            for (uint32_t i = 0; i < blockCount; i++) {
                AscendCBisheng::DataCopy(dstTensor.data()[dstOffset + i * dstOuterStrideCol],
...
struct TileCopyTla<Arch::AtlasA2,
    ...
    void operator()(TensorDst const &dstTensor, TensorSrc const &srcTensor)
    {
        ...
        if (srcOuterStrideRow / ELE_NUM_PER_C0 <= STRIDE_LIMIT) {
            ...
            repeatParams.dstStride = 0;
            for (uint32_t i = 0; i < blockCount; i++) {
                AscendCBisheng::DataCopy(dstTensor.data()[dstOffset + i * dstOuterStrideRow],
...
template <class ArchTag, class Element>
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::VectorLayout, AscendCBisheng::TPosition::GM>,
    Gemm::GemmType<Element, layout::VectorLayout, AscendCBisheng::TPosition::A1>> {
