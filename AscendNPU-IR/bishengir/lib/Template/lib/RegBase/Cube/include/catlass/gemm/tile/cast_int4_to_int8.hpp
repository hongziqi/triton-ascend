...
struct TileCastInt4ToInt8 {
    ...
    TileCastInt4ToInt8(Arch::Resource<ArchTag> const &resource, Params const &params) {
        if constexpr (g_coreType == AscendC::AIV) {
            ...
            for (uint32_t i = 0; i < STAGES; i++) {
                ...
                ubEventList[i] = i;
                AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::V_MTE2>(ubEventList[i]);
                AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::MTE3_S>(ubEventList[i]);
            }
        }
    }
    ...
    ~TileCastInt4ToInt8() {
        if constexpr (g_coreType == AscendC::AIV) {
            for (uint32_t i = 0; i < STAGES; i++) {
                AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::V_MTE2>(ubEventList[i]);
                AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::MTE3_S>(ubEventList[i]);
            }
        }
    }
    ...
    )
    {
        ...
        for (uint32_t loopIdx = 0; loopIdx < loops; loopIdx++) {
            ...
            AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::V_MTE2>(ubEventList[pingpong]);
            AscendC::DataCopyPad(ubInTensorList[pingpong],
                gmSrc[taskOffsetSrc + tileOffsetSrc], dataCopyParamsIn, padParams);

            AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::MTE2_S>(ubEventList[pingpong]);
            AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::MTE2_S>(ubEventList[pingpong]);

            AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::MTE3_S>(ubEventList[pingpong]);

            AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::S_V>(ubEventList[pingpong]);
            AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::S_V>(ubEventList[pingpong]);
            ...
            AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::V_MTE2>(ubEventList[pingpong]);

            AscendC::Cast(ubOutTensorList[pingpong], ubWorkspaceList[pingpong],
                AscendC::RoundMode::CAST_NONE, actualTiles * tileLenRoundInt4);

            AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::V_MTE3>(ubEventList[pingpong]);
            AscendCBisheng::WaitFlag<AscendCBisheng::HardEvent::V_MTE3>(ubEventList[pingpong]);
            ...
            AscendCBisheng::SetFlag<AscendCBisheng::HardEvent::MTE3_S>(ubEventList[pingpong]);
