...
def discover() -> list[Manifest]:
    ...
    for manifest in sorted(REPO_ROOT.rglob("pyproject.toml")):
        ...
        if _out_of_place(manifest):
            # Legacy Triton fork trees may ship unrelated pyproject.toml files
            # during migration to the out-of-tree layout; skip them.
            LOG.warning("skipping extension manifest in unexpected location: %s",
                        manifest)
            continue
        cfg = load(manifest)
